...
C'est une suite d'étapes qui va s'éxécuter après le push d'un code dans un repository. Généralement, un CI service se déclenchera après le push et exécutera un script dans un CI serveur.

Par exemple, imaginons qu'une équipe de 2 developpeurs veulent travailler sur un projet et qu'ils ont mis en place un systeme d'intégration continue. Lorsqu'ils vont push leur code sur un repository, un CI service(gitHub Action, Jenkins etc...) sera déclenché. Ce service entrainera une suite d'actions qui va lancer des commandes à partir du code qui été push. Génréralement le code sera build et testé sur dans serveur pour voir si tout fonctionne correctement.

#### CD(Continuous Delivery):
C'est une extension de la Continuous Integration. Elle permet de rendre certains processus de deploiement automatiques. Cette étape n'est pas à 100% automatique. L'homme peut encore intervenir dans une des étapes du déploiement.  
...
Jenkins est un outil open souce d'intégration continu (CI), qui a été developpé en java. En utilisant Jenkins, on va créer une pipeline (une suite d'étapes) à
travers des configurations qu'il faudra créer. Ces configurations sont appelées des Jobs.
...
#### 4 - Installer les plugins recommandés par Jenkins  
...
Allez dans "Administrer Jenskins", puis dans "Gestion des plugins"
![](images/img001.png)
![](images/img002.png)
...
Laissez la même configuration comme l'image ci-dessus. Appuyez sur "Appliquer" puis "Enregistrer"
...
Si votre projet est en private, ajoutez vos identifiants et mot de passe en cliquant sur "Ajouter" à côté de "Credentials"

Dans la partie "Build", nous allons utiliser les commandes Maven suivantes : clean war:war test.
Ces commandes permettent de supprimer les anciens fichiers générés, de générer le .war et d'executer les tests unitaires.
Indiquez également le nom de votre fichier POM à la racine (par défaut, pom.xml)

![](images/img009.png) 

Dans la partie "Post Steps", nous allons déterminer les étapes que nous allons réaliser une fois que le build aura été fait. Dans notre cas, copier le .war vers un dossier.
...
Dans la partie inférieure gauche de la page, vous retrouvez l'historique de vos build ainsi que le build que vous venez de lancer. Cliquez sur ce lien (voir image)
...
Sur cette page vous pourrez voir toutes les étapes par lesquelles passe Jenkins. Sur les images ci-dessous, vous verrez les étapes importantes du build (récupération du code source sur Github / GitLab, génération du .war, execution des tests unitaires, appel de la commande bash pour copier le fichier).
![](images/img014.png)
![](images/img015.png)
![](images/img016.png)

(Dans la partie "génération du .war" (encadrée en rouge) vous voyez le path où a été généré le .war. Si celui-ci diffère de l'exemple, vous pouvez modifier votre configuration de build en cliquant sur le bouton "Configuration"
...
3) Allez dans le gestionnaire des services windows (tapez "service" dans la barre de recherche windows et vous devriez le trouver). Cherchez le service "Jenkins".
Puis le redémarrer.
...
Une fois la configuation mise en place, cette technologie permet aux équipes de développeurs de se concentrer sur leur expertise sans se soucier des contraintes de déploiement.
